...
- **Remote gateway OAuth flow**: When the user is on a Hermes Desktop app connected to a remote gateway, the OAuth callback URL goes to localhost on the server, unreachable from the user's browser. See `references/remote-oauth-flow.md` for the callback-forwarding technique.
