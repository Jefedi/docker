...
Copiez ce fichier dans ~/.hermes/plugins/dl-video/__init__.py
Puis activez dans config.yaml: plugins.enabled: [..., "dl-video"]
"""
...
async def _dl_handler(raw_args: str) -> str:
    ...
    if not url.startswith(("http://", "https://")):
        return f"❌ `{url}` n'est pas une URL valide."
    ...
    try:
        ...
        result = subprocess.run(cmd_parts, capture_output=True, text=True, timeout=65)
        if result.returncode != 0:
            return f"❌ Erreur MeTube (exit={result.returncode}): `{result.stderr[:200]}`"
        response = json.loads(result.stdout)
        if response.get("status") == "ok":
            return f"✅ Vidéo ajoutée à MeTube !\n`{url}`\n\nLe téléchargement est en cours."
