...
for f in sorted(set(glob.glob('/opt/data/tmp/veille/lp_p*.html') + ['/opt/data/tmp/veille/lp.html'])):
    ...
    for card in cards[1:]:
        ...
        price = 0
        price_m = re.search(r'<span class="prix">([^<]+)€</span>', card)
        if price_m:
        ...
        if not price:
            ...
            if price_m2:
                ps = price_m2.group(1).replace('&nbsp;', '').replace(' ', '').replace('.', '').replace('\xa0', '')
                try: price = int(ps)
                except: pass
...
print(f"Le-Partenaire T2+ listings (all prices): {len(listings)}")
for l in listings:
    print(f"  {l['id']} | T{l['pieces']} {l['surface']}m² | {l['price']}€ | DPE={l['dpe']}")
    print(f"    {l['desc'][:120]}")
