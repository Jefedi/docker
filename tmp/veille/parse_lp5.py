...
for f in sorted(set(glob.glob('/opt/data/tmp/veille/lp_p*.html') + ['/opt/data/tmp/veille/lp.html'])):
    ...
    raw = open(f).read()
    html = htmllib.unescape(raw)  # convert &nbsp; to space, etc.
...
for l in listings:
    flag = " *** MATCH ***" if (l['pieces']>=2 and l['surface']>=28 and 0<l['price']<=500) else ""
    print(f"  {l['id']} | T{l['pieces']} {l['surface']}m² | {l['price']}€ | DPE={l['dpe']}{flag}")
    print(f"    {l['desc'][:120]}")
