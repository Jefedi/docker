...
for page in range(1, 6):
    ...
    for i, (h2_text, link) in enumerate(zip(h2s, seen_links)):
        ...
        # Extract price - use class="prix" pattern
        id_pos = raw.find(f'{listing_id}')
        price = 0
        description = ''
        if id_pos >= 0:
            chunk = raw[id_pos:id_pos+5000]
            # Price pattern: class="prix">XXX&nbsp;€<
            price_match = re.search(r'class="prix"[^>]*>(\d[\d\s\xa0]*)\s*€', chunk)
            if price_match:
            ...
            # If price still 0, try to find it in description
            if price == 0 and description:
                # Look for "proposé à XXX €" or "Loyer ... XXX €" or "XXX €/mois" 
                desc_price = re.search(r'(?:proposé à|Loyer\s*(?:charges comprises|hors charges)?\s*:\s*)(\d[\d\s\xa0]*)\s*€', description, re.I)
                if desc_price:
                    price_str = re.sub(r'[\s\xa0]', '', desc_price.group(1))
                    try:
                        price = int(price_str)
                    except:
                        pass
...
print("\n=== ALL T2+ with prices extracted ===")
for l in all_listings:
    if l['rooms'] >= 2 and l['price'] > 0:
        tag = "✅" if l['price'] <= 500 and l['surface'] >= 28 else "❌"
        print(f"  {tag} {l['id']}: {l['price']}€ {l['surface']}m² {l['rooms']}p")

# Show T2+ with price 0
print(f"\n=== T2+ with price 0 (not extracted): {sum(1 for l in all_listings if l['rooms']>=2 and l['price']==0)} ===")